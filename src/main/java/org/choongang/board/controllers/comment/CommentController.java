...
public class CommentController implements ExceptionProcessor {
    ...
    public String save(@Valid RequestComment form, Errors errors, Model model) {
        ...
        if(errors.hasErrors()) {
            ...
            throw new AlertException(Utils.getMessage(error.getCodes()[0]),
                HttpStatus.BAD_REQUEST);
        }

        CommentData commentData = commentSaveService.save(form); // 댓글 저장, 수정

        String script = String.format("parent.location.replace('/board/view/%d?comment_id=%d');",
            commentData.getBoardData().getSeq(), commentData.getSeq());
